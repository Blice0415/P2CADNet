...
from .model_utils import _make_seq_first, _make_batch_first, \
    _get_padding_mask, _get_key_padding_mask, _get_group_mask, _generate_square_subsequent_mask
...
class CADEmbedding(nn.Module):
    ...
    def __init__(self, cfg, seq_len, use_group=False, group_len=None):
        ...
        self.command_embed = nn.Embedding(cfg.n_commands, cfg.d_model) #(6,256)

        args_dim = cfg.args_dim + 1
        self.arg_embed = nn.Embedding(args_dim, 64, padding_idx=0)#(257,64,0)
        self.embed_fcn = nn.Linear(64 * cfg.n_args, cfg.d_model)#(64*16,256)
...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        # print("src:",src.size())
        out = self.decoder(src, z, tgt_mask=_generate_square_subsequent_mask(60).cuda(), tgt_key_padding_mask=None)
        # print("out:",out.size())
...
class CADTransformer(nn.Module):
    ...
    def forward(self, commands_enc, args_enc,
        ...
        # print("commands_enc_:",commands_enc_.size())
        # print("args_enc_:",args_enc_.size())
        if z is None:
            z = self.encoder(commands_enc_, args_enc_)
            # print("z_encoder:",z.size())
            z = self.bottleneck(z)
            # print("z_botn:",z.size())
        ...
        # print("z4:",out_logits[0].size())
        # print("z4:",out_logits[1].size())
        out_logits = _make_batch_first(*out_logits)
        # print("z5:",out_logits[0].size())
        # print("z5:",out_logits[1].size())
        ...
        # print("tgt:",res["tgt_commands"].size())
        # print("tgt:",res["tgt_args"].size())
        return res
