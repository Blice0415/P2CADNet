...
def CADsolid2pc(shape, n_points, name=None):
    ...
    # out_mesh.export(file_obj="tmp_out_{}.obj".format(name),file_type='obj')
    # os.system("rm tmp_out_{}.stl".format(name)) # linux
    os.remove("tmp_out_{}.stl".format(name)) # windows
    out_pc, _ = sample_surface(out_mesh, n_points)
    return out_pc

def CADsolid2mesh(shape, n_points, name=None):
    """convert opencascade solid to mesh"""
    bbox = Bnd_Box()
    brepbndlib_Add(shape, bbox)
    if bbox.IsVoid():
        raise ValueError("box check failed")
